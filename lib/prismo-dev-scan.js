...
Commands:
  ...
  feedback    Print a local copy-paste feedback packet for user testing.
...
Options:
  ...
  --dry-run  Preview doctor/fix/optimize actions without writing files.
...
Usage:
  prismo optimize [auth|frontend|backend] [--json] [--dry-run] [path]

Examples:
  prismo optimize
  prismo optimize frontend
  prismo optimize --dry-run --json
...
Output:
  ...
  The firewall is a local policy file for the agent to follow before reading files; it does not enforce filesystem access by itself.`,
    feedback: `PrismoDev Feedback

Usage:
  prismo feedback [--json] [path]

Output:
  Prints a local-only feedback packet testers can paste into an issue or message.
  This command does not upload telemetry, read local logs, or make network calls.`,
    ci: `Prismo CI
...
async function runCli(argv) {
  ...
  if (!["dev", "init", "doctor", "firewall", "feedback", "setup", "scan", "optimize", "context", "cc", "usage", "watch", "demo"].includes(command)) {
    throw new Error(`Unknown command: ${command}. Try: prismo doctor, prismo watch, prismo firewall, prismo feedback, prismo init, prismo scan, prismo optimize, prismo context, prismo cc, or prismo usage`);
  }
  ...
  if (command === "dev") {
    ...
    const result = runDevFlow(target, {
      json,
      limit: parsePositiveInt(parsed.values.limit, 3),
    });
    if (json) {
  ...
  if (command === "init") {
    ...
    const result = runInit(target, { dryRun: parsed.flags.dryRun });
    if (json) {
  ...
  if (command === "doctor") {
    ...
    const result = runDoctor(target || process.cwd(), {
      json,
      limit: parsePositiveInt(parsed.values.limit, 3),
      scope,
  ...
  if (command === "firewall") {
    ...
    const result = runFirewall(target || process.cwd(), {
      task: scope || "general",
      scope,
      dryRun: parsed.flags.dryRun,
    });
  ...
  if (command === "feedback") {
    const json = parsed.flags.json;
    const target = parsed.positionals[0] || process.cwd();
    const result = runFeedback(target);
    if (json) {
      console.log(JSON.stringify(result, null, 2));
      return;
    }
    console.log(renderFeedbackTerminal(result));
    return;
  }
  ...
  if (command === "setup") {
    ...
    const result = await runSetup(target, {
      limit: parsePositiveInt(parsed.values.limit, 3),
      proxyUrl: parsed.values.proxyUrl || DEFAULT_PRISMO_PROXY_URL,
  ...
  if (command === "cc") {
    ...
    const limit = subcommand === "list"
      ? parsePositiveInt(parsed.values.limit, 10)
      : subcommand === "last"
        ? lastCount
        : 1;
  ...
  if (command === "usage" || command === "watch") {
    ...
    const limit = parsePositiveInt(parsed.values.limit, 5);
    const intervalMs = parsed.values.intervalMs;
    const tokenBudget = parsed.values.tokenBudget || (auto ? 600000 : null);
    const usageOptions = {
      tool,
      cwd: path.resolve(target),
      limit,
  ...
  if (command === "context") {
    const json = parsed.flags.json;
    const { scope, target } = parseScopeAndTarget(rest);
    const ctx = createOptimizeContext(target || process.cwd(), scope);
  ...
  if (command === "optimize") {
    const json = parsed.flags.json;
    const { scope, target } = parseScopeAndTarget(rest);
    const result = runOptimize(target || process.cwd(), { scope, dryRun: parsed.flags.dryRun });
    if (json) {
      console.log(JSON.stringify(result, null, 2));
      return;
    }
  ...
  const fix = parsed.flags.fix;
  const noReport = parsed.flags.noReport;
  const json = parsed.flags.json;
  const simple = parsed.flags.simple;
  const ciMode = parsed.flags.ci;
  const includeUsage = parsed.flags.usage;
  ...
  const result = scanRepo(target, {
    includeUsage,
    usageLimit: parsePositiveInt(parsed.values.limit, 5),
    usageTool: parsed.values.usageTool || "all",
  });
  scanDone();
