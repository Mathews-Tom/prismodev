...
function renderTerminalReport(result, options = {}) {
  ...
  lines.push(`- Claude Code: ${result.agentReadiness.claudeCode.detected ? "detected" : "not detected"}; logs: ${result.agentReadiness.claudeCode.localLogsFound ? "found" : "not found"}; project MCP: ${result.agentReadiness.claudeCode.projectMcpServers}; global MCP: ${result.agentReadiness.claudeCode.globalMcpServers}; project hooks: ${result.agentReadiness.claudeCode.projectHooks}; global hooks: ${result.agentReadiness.claudeCode.globalHooks}`);
  lines.push(`- Codex: ${result.agentReadiness.codex.detected ? "detected" : "not detected"}; logs: ${result.agentReadiness.codex.localLogsFound ? "found" : "not found"}; project MCP: ${result.agentReadiness.codex.projectMcpServers}; global MCP: ${result.agentReadiness.codex.globalMcpServers}`);
  ...
  lines.push(`- Project Claude hooks: ${stack.claudeHooks}; project MCP servers: ${stack.mcpServerTotal}; global MCP servers: ${stack.globalMcpServerTotal}`);
  lines.push("");
  ...
  if (!result.issues.length) {
    ...
    for (const issue of result.issues.slice(0, 8)) {
      ...
      lines.push(`  Evidence: ${issue.scope}; ${issue.confidence}`);
...
function renderMarkdownReport(result) {
  ...
  lines.push(`  - Project MCP servers: ${result.agentReadiness.claudeCode.projectMcpServers}; global MCP servers: ${result.agentReadiness.claudeCode.globalMcpServers}`);
  lines.push(`  - Project hooks: ${result.agentReadiness.claudeCode.projectHooks}; global hooks: ${result.agentReadiness.claudeCode.globalHooks}`);
  ...
  lines.push(`  - Project MCP servers: ${result.agentReadiness.codex.projectMcpServers}; global MCP servers: ${result.agentReadiness.codex.globalMcpServers}`);
  ...
  lines.push(`- Project Claude hooks: ${result.optimizationStack.claudeHooks}`);
  lines.push(`- Project MCP/tool servers: ${result.optimizationStack.mcpServerTotal}`);
  lines.push(`- Global MCP/tool servers: ${result.optimizationStack.globalMcpServerTotal}`);
  lines.push("");
  ...
  lines.push(`- Project Claude config files found: ${result.claudeConfig.project.files.length ? result.claudeConfig.project.files.map((file) => `\`${file}\``).join(", ") : "none"}.`);
  lines.push(`- Global Claude config files found: ${result.claudeConfig.global.files.length ? result.claudeConfig.global.files.map((file) => `\`${file}\``).join(", ") : "none"}.`);
  lines.push(`- Project MCP servers: ${result.claudeConfig.project.mcpServers}. Global MCP servers: ${result.claudeConfig.global.mcpServers}. Total MCP servers: ${result.claudeConfig.mcpServers}.`);
  lines.push(`- Project hooks: ${result.claudeConfig.project.hooks}. Global hooks: ${result.claudeConfig.global.hooks}. Plugin/skill references: ${result.claudeConfig.pluginRefs}.`);
  ...
  lines.push(`- Project Codex config files found: ${result.codexConfig.project.files.length ? result.codexConfig.project.files.map((file) => `\`${file}\``).join(", ") : "none"}.`);
  lines.push(`- Global Codex config files found: ${result.codexConfig.global.files.length ? result.codexConfig.global.files.map((file) => `\`${file}\``).join(", ") : "none"}.`);
  lines.push(`- Project Codex MCP/tool references: ${result.codexConfig.project.mcpServers}. Global Codex MCP/tool references: ${result.codexConfig.global.mcpServers}. Total: ${result.codexConfig.mcpServers}.`);
