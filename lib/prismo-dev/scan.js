...
function createClaudeConfigBucket() {
  return { files: [], mcpServers: 0, hooks: 0, pluginRefs: 0 };
}

function createCodexConfigBucket() {
  return { files: [], mcpServers: 0 };
}

function mergeClaudeConfigBuckets(project, global) {
  return {
    files: [...project.files, ...global.files],
    mcpServers: project.mcpServers + global.mcpServers,
    hooks: project.hooks + global.hooks,
    pluginRefs: project.pluginRefs + global.pluginRefs,
  };
}

function mergeCodexConfigBuckets(project, global) {
  return {
    files: [...project.files, ...global.files],
    mcpServers: project.mcpServers + global.mcpServers,
  };
...
function scanClaudeConfig(root) {
  const candidates = [
    { scope: "project", filePath: path.join(root, ".claude", "settings.json") },
    { scope: "project", filePath: path.join(root, ".claude.json") },
    { scope: "global", filePath: path.join(os.homedir(), ".claude", "settings.json") },
    { scope: "global", filePath: path.join(os.homedir(), ".claude.json") },
  ];
  const project = createClaudeConfigBucket();
  const global = createClaudeConfigBucket();

  for (const { scope, filePath } of candidates) {
    if (!fs.existsSync(filePath)) continue;
    const text = readIfText(filePath);
    if (!text) continue;
    const bucket = scope === "project" ? project : global;
    const rel = filePath.startsWith(root) ? normalizeRel(path.relative(root, filePath)) : filePath;
    bucket.files.push(rel);
    try {
      const json = JSON.parse(text);
      bucket.mcpServers += countJsonObjectKeys(json, "mcpServers");
      bucket.hooks += countJsonObjectKeys(json, "hooks");
    } catch {
      bucket.mcpServers += (text.match(/mcpServers|mcp_servers|mcp-server/g) || []).length;
      bucket.hooks += (text.match(/hooks|hook/g) || []).length;
    }
    bucket.pluginRefs += (text.match(/plugin|skill/gi) || []).length;
  }

  const total = mergeClaudeConfigBuckets(project, global);
  return { ...total, project, global, total };
}

function scanCodexConfig(root) {
  const candidates = [
    { scope: "project", filePath: path.join(root, ".codex", "config.toml") },
    { scope: "project", filePath: path.join(root, "AGENTS.md") },
    { scope: "global", filePath: path.join(os.homedir(), ".codex", "config.toml") },
  ];
  const project = createCodexConfigBucket();
  const global = createCodexConfigBucket();
  ...
  for (const { scope, filePath } of candidates) {
    ...
    if (!text) continue;
    const bucket = scope === "project" ? project : global;
    bucket.files.push(filePath.startsWith(root) ? normalizeRel(path.relative(root, filePath)) : filePath);
    bucket.mcpServers += (text.match(/\[mcp|mcp_servers|mcp-server|server\]/gi) || []).length;
  }
  const total = mergeCodexConfigBuckets(project, global);
  return { ...total, project, global, total };
}
...
function detectOptimizationStack(root, claudeConfig, codexConfig) {
  ...
  return {
    tools,
    claudeHooks: claudeConfig.project.hooks,
    globalClaudeHooks: claudeConfig.global.hooks,
    totalClaudeHooks: claudeConfig.total.hooks,
    claudeMcpServers: claudeConfig.project.mcpServers,
    globalClaudeMcpServers: claudeConfig.global.mcpServers,
    totalClaudeMcpServers: claudeConfig.total.mcpServers,
    codexMcpServers: codexConfig.project.mcpServers,
    globalCodexMcpServers: codexConfig.global.mcpServers,
    totalCodexMcpServers: codexConfig.total.mcpServers,
    claudePluginDetected: projectClaudePlugin,
    mcpServerTotal: claudeConfig.project.mcpServers + codexConfig.project.mcpServers,
    globalMcpServerTotal: claudeConfig.global.mcpServers + codexConfig.global.mcpServers,
    totalMcpServerTotal: claudeConfig.total.mcpServers + codexConfig.total.mcpServers,
...
function detectAgentReadiness(root, claudeConfig, codexConfig, realUsage) {
  ...
  return {
    claudeCode: {
      ...
      projectConfigFiles: claudeConfig.project.files,
      globalConfigFiles: claudeConfig.global.files,
      localLogsFound: claudeSessionFiles.length > 0 || usageSources.has("claude-code"),
      mcpServers: claudeConfig.mcpServers,
      projectMcpServers: claudeConfig.project.mcpServers,
      globalMcpServers: claudeConfig.global.mcpServers,
      hooks: claudeConfig.hooks,
      projectHooks: claudeConfig.project.hooks,
      globalHooks: claudeConfig.global.hooks,
    ...
    codex: {
      ...
      projectConfigFiles: codexConfig.project.files,
      globalConfigFiles: codexConfig.global.files,
      localLogsFound: codexSessionFiles.length > 0 || usageSources.has("codex"),
      mcpServers: codexConfig.mcpServers,
      projectMcpServers: codexConfig.project.mcpServers,
      globalMcpServers: codexConfig.global.mcpServers,
...
function renderSetupTerminal(result) {
  ...
  lines.push("");
  lines.push("Detected:");
  lines.push(`- Claude Code: ${result.detected.claudeCode.detected ? "detected" : "not detected"}; logs: ${result.detected.claudeCode.localLogsFound ? "found" : "not found"}; project MCP: ${result.detected.claudeCode.projectMcpServers}; global MCP: ${result.detected.claudeCode.globalMcpServers}; project hooks: ${result.detected.claudeCode.projectHooks}; global hooks: ${result.detected.claudeCode.globalHooks}`);
  lines.push(`- Codex: ${result.detected.codex.detected ? "detected" : "not detected"}; logs: ${result.detected.codex.localLogsFound ? "found" : "not found"}; project MCP: ${result.detected.codex.projectMcpServers}; global MCP: ${result.detected.codex.globalMcpServers}`);
...
function defaultIssueEvidence(category) {
  if (category === "repo_size") return { scope: "project", confidence: "repo-heuristic" };
  return { scope: "project", confidence: "repo-heuristic" };
}

function addIssue(issues, severity, category, title, description, recommendation, estimatedTokenImpact = null, evidence = {}) {
  const defaultEvidence = defaultIssueEvidence(category);
  issues.push({
    severity,
    category,
    scope: evidence.scope || defaultEvidence.scope,
    confidence: evidence.confidence || defaultEvidence.confidence,
    title,
    description,
    recommendation,
...
function buildRecommendations({ hasClaudeIgnore, gitignorePatterns, exposedHighRiskDirs, largeFiles, instructionFiles, claudeConfig, toolOutputRisk, agentReadiness }) {
  ...
  if (claudeConfig.project.mcpServers >= 5) {
...
function toJsonPayload(result) {
  const payload = {
    ...
    claudeFindings: {
      ...
      projectConfigFiles: result.claudeConfig.project.files,
      globalConfigFiles: result.claudeConfig.global.files,
      projectScope: "project",
      projectConfidence: "repo-heuristic",
      globalScope: "environment",
      globalConfidence: "environment-heuristic",
      projectMcpServers: result.claudeConfig.project.mcpServers,
      globalMcpServers: result.claudeConfig.global.mcpServers,
      mcpServers: result.claudeConfig.mcpServers,
      projectHooks: result.claudeConfig.project.hooks,
      globalHooks: result.claudeConfig.global.hooks,
      hooks: result.claudeConfig.hooks,
      projectPluginRefs: result.claudeConfig.project.pluginRefs,
      globalPluginRefs: result.claudeConfig.global.pluginRefs,
      pluginRefs: result.claudeConfig.pluginRefs,
      project: result.claudeConfig.project,
      global: result.claudeConfig.global,
      total: result.claudeConfig.total,
    },
    codexFindings: {
      configFiles: result.codexConfig.files,
      projectConfigFiles: result.codexConfig.project.files,
      globalConfigFiles: result.codexConfig.global.files,
      projectScope: "project",
      projectConfidence: "repo-heuristic",
      globalScope: "environment",
      globalConfidence: "environment-heuristic",
      projectMcpServers: result.codexConfig.project.mcpServers,
      globalMcpServers: result.codexConfig.global.mcpServers,
      mcpServers: result.codexConfig.mcpServers,
      project: result.codexConfig.project,
      global: result.codexConfig.global,
      total: result.codexConfig.total,
...
function addRealUsageIssues(issues, usage) {
  ...
  if (total >= 1000000) {
    addIssue(
      ...
      { scope: "session", confidence: exact ? "exact-local-log" : "estimated-local-log" }
    );
  }
  ...
  if (toolTokens >= 50000) {
    addIssue(
      ...
      { scope: "session", confidence: usage.totals.exactTokens ? "exact-local-log" : "estimated-local-log" }
    );
  }
...
function scanRepo(rootDir = process.cwd(), options = {}) {
  ...
  if (claudeConfig.project.mcpServers >= 5) {
    addIssue(
      ...
      "medium",
      "mcp_tooling",
      `${claudeConfig.project.mcpServers} project MCP servers detected in Claude config`,
      "Many project-owned MCP servers can increase tool overhead and agent search space.",
      "Disable MCP servers not needed for the current repo.",
      estimateMcpImpact(claudeConfig.project.mcpServers)
    );
  }

  if (claudeConfig.project.hooks >= 5) {
    addIssue(
      issues,
      "low",
      "claude_config",
      `${claudeConfig.project.hooks} project Claude hooks detected`,
      "Large project-owned hook setups can add workflow overhead or noisy tool results.",
  ...
  if (codexConfig.project.mcpServers >= 5) {
    addIssue(
      issues,
      "medium",
      "codex_config",
      `${codexConfig.project.mcpServers} project MCP/tool references detected in Codex config`,
      "Large project-owned tool surfaces can add overhead in OpenAI/Codex workflows.",
      "Keep Codex tools scoped to the task.",
      estimateMcpImpact(codexConfig.project.mcpServers)
    );
  }
  ...
  if (!repoDetected) {
    addIssue(
      ...
      { scope: "setup", confidence: "repo-heuristic" }
    );
  }
